<template>
  <div class="basicinfo_box">
    ...
    <h3 class="basic_title"> CAT评估得分
      <span class="basic_title_mark "
            style="color: red;">注：共8个问题，数字0—5表示严重程度，请标记最能反映患者当前情况的选项选分，每个问题只能标记1个选项</span>
    </h3>
    <div class="basic_cont">
      <el-form :label-position="labelPosition"
        ...
        <el-form-item label="从不咳嗽">
          <el-radio-group v-model="basicInfo.catCough">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.examineList"
        ...
        <el-form-item label="爬坡或上一层楼时，没有气喘的感觉">
          ...
          <span class="examine_radio_suffix"> 爬坡或上一层楼时，感觉严重喘不过气</span>
        </el-form-item>
        <el-form-item label="在家里做任何事情">
          ...
          <span class="examine_radio_suffix"> 在家里做任何事情都很受影响</span>
        </el-form-item>
        <el-form-item label="尽管有肺部疾病，但对外出很有信心">
          ...
          <span class="examine_radio_suffix"> 由于有肺部疾病，对离开家一点信心都没有</span>
        </el-form-item>
        <el-form-item label="睡眠非常好">
          ...
          <span class="examine_radio_suffix"> 由于有肺部疾病，睡眠相当差</span>
        </el-form-item>
        ...
        <el-form-item label="总得分"
          ...
          <el-radio-group v-model="totalScore">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.totalScore"
      ...
      <el-form :label-position="labelPosition"
        ...
        <el-form-item label="">
          <el-radio-group v-model="basicInfo.mmrc">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.mMRCLevel"
                      :key="index">{{item.label}}级
    ...
    <div class="basic_cont basic_level">
      <div class="basic_headings">mMRC评分，询问过去一年中急性加重次数？<br />
        0级-只有在剧烈活动时感到呼吸困难;<br />1级-在平地快步行走或步行爬小坡时出现气短;<br /> 2级-由于气短，平地行走时比同龄人慢或者需要停下来休息;<br /> 3级-在平地行走约100米或数分钟后需要停下来喘气;<br /> 4级-因为严重呼吸困难而不能离开家，或在穿脱衣服时出现呼吸困难</div>
      <el-form :label-position="labelPosition"
               :model="basicInfo"
               :rules="rules"
               class="form_box"
        ...
        <el-form-item label="">
          <el-radio-group v-model="basicInfo.isCountGtTwo">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.mMRscore"
        ...
        <el-form-item label=""
          ...
          <el-button type="primary"
                     @click="updateQuestionnaire">保存</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
import { updateQuestionnaire } from '@/service/api'
export default {
  props: {
    info: {
      type: Object,
