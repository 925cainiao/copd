<template>
  <div>
      ...
      <el-container v-show="tagPlan">
        ...
        <el-container>
          ...
          <el-main class="drug_plan_main">
            <div>
              <base-plan :basicPlan="basicPlan"
                         :frequencyOptions="frequencyOptions"
                         :drugUsageOptions="drugUsageOptions"
                         :showStep="showStep"
                         ref="basePlan"></base-plan>
              <edit-plan :basicPlan="showInfo"
                         :queryDrugInfoOption="queryDrugInfoOption"
                         :frequencyOptions="frequencyOptions"
                         :drugUsageOptions="drugUsageOptions"
                         :showStep="showStep"
                         ref="editPlan"></edit-plan>
              <plan-list :showInfo="showInfo.scheme"
                         :showStep="showStep"
              ...
              <el-row class="button_align">
                <el-col :span="24">
                  <el-button class="el-icon-back"
                             @click="goBack"
                             :disabled="drugPlan.length === 0 && showStep === 0">上一步</el-button>
    ...
    <timeline-drug :drugPlan.sync="drugPlan"
                   :isShowDrug.sync="isShowDrug"
                   :info="info"></timeline-drug>

  </div>
</template>
<script>
import { querySchemeGroup, queryDrugInfo, querySchemeTag, queryDrugFrequency, queryDrugUsage, saveScheme, schemePlanId } from '@/service/api'
...
export default {
  components: {
    BasePlan: () => import(/* webpackChunkName: "BasePlan" */ './components/BasePlan'),
    EditPlan: () => import(/* webpackChunkName: "EditPlan" */ './components/EditPlan'),
    PlanList: () => import(/* webpackChunkName: "PlanList" */ './components/PlanList'),
    TimelineDrug: () => import(/* webpackChunkName: "TimelineDrug" */ './components/TimelineDrug')
  },
  props: {
    info: {
      type: Object,
  ...
  methods: {
      ...
      schemePlanId(`/${_this.info.id}`, {}).then((res) => {
        if (res.status === '0') {
          if (res.data && res.data.length > 0) {
            ...
            _this.isShowDrug = true
          } else {
            _this.isShowDrug = false
          }
        }
      })
    },
    ...
    save () {
      ...
      if (_this.showStep === 2) {
        ...
        let params = {
          doctorId: _this.doctorInfo.doctorId,
