<template>
  <div class="basicinfo_box">
    <el-row class="basic_header">
      ...
      <el-col :span="12"
              class="basic_header_btn">
        <div class="display_inline_block"
             v-show="isReadOnly">
          <el-button type="primary"
                     @click="isReadOnly = !isReadOnly"
                     v-show="isReadOnly || step === 0">编辑</el-button>
        </div>
        <div class="display_inline_block"
             v-show="!isReadOnly">
          <el-button type="primary"
        ...
        <div class="display_inline_block">
          <el-button type="primary"
                     @click="addBtn"
                     v-show="isReadOnly || step === 1">新建</el-button>
        </div>
      </el-col>
    </el-row>
    <div class="basic_cont">
      <el-form :label-position="labelPosition"
               ...
               :model="basicInfo"
               :rules="rules"
               class="form_box form_basicinfo"
               :label-width="labelWidth"
               :disabled="isReadOnly"
               ref="basicInfo">
        <el-form-item label="医院"
                      prop="registHospital">
          <el-input v-model="basicInfo.registHospital"
                    placeholder=""
                    readonly></el-input>
        </el-form-item>
        <el-form-item label="首诊医生">
          <el-input v-model="basicInfo.doctor"
        ...
        <el-form-item label="性别"
          ...
          <el-select v-model="basicInfo.gender"
            ...
            <el-option :label="item.value"
                       :value="item.label"
                       v-for="(item,index) in $dict.genderList"
                       :key="index"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="年龄"
                      ...
                      prop="age">
          <el-input v-model="basicInfo.age"
                    placeholder=""></el-input>
        </el-form-item>
        <el-form-item label="身份证号码"
                      prop="idCard">
          <el-input v-model="basicInfo.idCard"
                    placeholder=""
        ...
        <el-form-item label="居住地所属"
                      prop="addressType"
                      class="width_all">
          <el-select v-model="basicInfo.addressType"
                     placeholder="">
        ...
        <el-form-item label="联系电话"
                      prop="contactPhone">
          <el-input v-model="basicInfo.contactPhone"
                    placeholder=""
                    maxlength="11"></el-input>
        </el-form-item>
        <el-form-item label="学历"
                      prop="degree">
          <el-select v-model="basicInfo.degree"
                     placeholder="">
            <el-option :label="item.label"
                       :value="item.value"
                       v-for="(item,index) in $dict.degreeList"
                       :key="index"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="职业"
                      prop="career">
          <el-select v-model="basicInfo.career"
                     placeholder="">
            <el-option label="在职或者参加劳动"
                       value="在职或者参加劳动"></el-option>
            <el-option label="离退休"
                       value="离退休"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="家庭月均收入"
                      prop="monthlyFamilyIncomeEach"
                      class="width_all">
          <el-radio-group v-model="basicInfo.monthlyFamilyIncomeEach">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.monthlyFamilyIncomeEach"
                      :key="index">{{item.value}}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="家庭总人口"
                      class="width_all"
                      prop="familyPopulation">
          <el-input v-model="basicInfo.familyPopulation"
                    placeholder=""
                    style="width: 98px;"></el-input>
          <span class="examine_radio_suffix">(常住一起的家庭成员数量)</span>
        </el-form-item>
        <el-form-item label="民族"
                      prop="race">
          <el-select v-model="basicInfo.race"
                     placeholder="">
            <el-option label="城市"
                       value="1"></el-option>
            <el-option label="农村"
        ...
        <el-form-item label="医保类型"
                      prop="insuranceType"
                      class="form_checkbox basic_checkbox width_all">
          <el-radio-group v-model="basicInfo.insuranceType">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.insuranceCheckbox"
                      :key="index"
                      @change="insuranceTypeRadioChange"></el-radio>
          </el-radio-group>
          <el-input v-model="insuranceType"
                    class="indurance_style"
                    placeholder=""></el-input>
        </el-form-item>
        <el-form-item label="初次就诊日期"
                      prop="createTime">
          <el-date-picker v-model="basicInfo.firstTime"
                          type="date"
    ...
    <h2 class="basic_title">未进行简易肺功能仪筛查的原因(若已进行筛查则不必填写)：</h2>
    <div class="basic_cont">
      <el-form label-position="left"
        ...
        <el-form-item label="是否愿意进行筛查？">
          <el-radio-group v-model="basicInfo.isScreening">
            <el-radio :label="item.label"
                      v-for="(item,index) in $dict.isScreening"
                      :key="index"
                      @change="screeningChange">{{item.value}}</el-radio>
            <!-- <el-radio label="0">不愿意</el-radio> -->
          </el-radio-group>
        </el-form-item>
        <el-form-item label="不愿意来筛查："
                      class="reason_checkbox"
                      v-show="basicInfo.isScreening === 0">
          <el-checkbox-group v-model="type">
            <el-checkbox :label="item.label"
                         name="type"
                         v-for="(item,index) in $dict.unwillingReason"
                         :key="index"></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <el-form-item label="愿意进行筛查，但未进行："
                      class="reason_checkbox"
                      v-show="basicInfo.isScreening === 1">
          <el-checkbox-group v-model="type">
            <el-checkbox :label="item.label"
                         name="type"
                         v-for="(item,index) in $dict.willingReason"
                         :key="index"></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
...
export default {
  ...
  watch: {
    info (n, o) {
      ...
      this.basicInfo.registHospital = this.doctorInfo.registHospital
    },
    basicInfo (n, o) {
  ...
  methods: {
    ...
    addBtn () {
      ...
      _this.$nextTick(() => {
        ...
        _this.$refs.basicInfo.insuranceType = ''
        _this.$refs.basicInfo.type = []
      })
    },
    // 确认按钮
    confirmBtn () {
